...
**Fallback philosophy.** Every agent defines a degrade-gracefully path *except* where a degraded output would actively mislead (e.g., Proposal Generator has no auto-fallback — a bad formal document is worse than none). Where fallback exists, it's stated per-agent below, not left implicit.
...
| Inputs | `lead.places_data` |
| Outputs | `business_analysis` row (`brand_brief`, `sentiment_summary`) |
...
| Inputs | `business_analysis.brand_brief`, `lead.category` |
