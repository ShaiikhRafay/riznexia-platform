...
function fakeLayoutConfigRow(overrides: Record<string, unknown> = {}) {
  return {
    ...
    responsiveRules: {
      ...
      stackedLayout: true,
      tapTargetSizePx: 48,
      perSection: { hero: 'reflow', menu: 'reflow', contact: 'reflow', footer: 'reflow' },
    },
...
describe('LayoutGenerationService', () => {
  let prisma: {
  ...
  let service: LayoutGenerationService;
  ...
  beforeEach(() => {
      ...
      $transaction: jest.fn((callback: (tx: unknown) => unknown) =>
        callback({
          layoutConfiguration: {
            ...
            create: jest.fn().mockResolvedValue(fakeLayoutConfigRow()),
          },
        }),
      ),
    };
    ...
    service = new LayoutGenerationService(
  ...
  describe('findLatestForLead', () => {
    ...
    it('returns the latest configuration, mapped to the API shape', async () => {
      ...
      expect(result).toMatchObject({ id: 'layout-config-1', layoutEngineVersion: 'v1.0' });
    });
  });

  describe('generateLayoutForLead', () => {
    it('throws LeadNotFoundException when the lead does not exist', async () => {
      leadsService.findById.mockResolvedValue(null);
      await expect(service.generateLayoutForLead('missing')).rejects.toBeInstanceOf(
        LeadNotFoundException,
      );
    });

    it('throws ThemeConfigurationNotFoundException when no theme configuration exists yet', async () => {
      prisma.themeConfiguration.findFirst.mockResolvedValue(null);
      await expect(service.generateLayoutForLead('lead-1')).rejects.toBeInstanceOf(
        ThemeConfigurationNotFoundException,
      );
    });

    it('returns cacheHit:true when a configuration already exists for this themeConfigurationId', async () => {
      prisma.themeConfiguration.findFirst.mockResolvedValue(fakeThemeConfigRow());
      prisma.layoutConfiguration.findFirst.mockResolvedValue(fakeLayoutConfigRow());

      const result = await service.generateLayoutForLead('lead-1');
    ...
    it('generates and persists a new configuration on a cache miss, using real deterministic generation', async () => {
      prisma.themeConfiguration.findFirst.mockResolvedValue(fakeThemeConfigRow());
      prisma.layoutConfiguration.findFirst.mockResolvedValue(null);
      prisma.businessAnalysis.findUniqueOrThrow.mockResolvedValue(fakeAnalysisRow());
      const txCreate = jest.fn().mockResolvedValue(fakeLayoutConfigRow());
      ...
      // Real generateLayout() output — hero is first, full-width; nav
      // excludes hero/footer.
      const data = txCreate.mock.calls[0][0].data;
      expect(data.pageStructure[0]).toMatchObject({
        sectionId: 'hero',
        order: 1,
        layoutType: 'full-width',
      });
      expect(data.navigation.items).toEqual(['menu', 'contact']);
    });
    ...
    it('allocates configVersion as max+1 within the transaction', async () => {
      ...
      prisma.layoutConfiguration.findFirst.mockResolvedValue(null);
      prisma.businessAnalysis.findUniqueOrThrow.mockResolvedValue(fakeAnalysisRow());
      const txCreate = jest.fn().mockResolvedValue(fakeLayoutConfigRow({ configVersion: 3 }));
      prisma.$transaction.mockImplementation((callback: (tx: unknown) => unknown) =>
        callback({
          layoutConfiguration: {
      ...
      await service.generateLayoutForLead('lead-1');
