...
import { ThemeDashboardPage } from './theme-dashboard-page';
...
// Select Lead (F7 Dashboard): `leadId` lives in the URL, the same
// bookmarkable/reload-safe pattern F6's Dashboard already established.
describe('ThemeDashboardPage', () => {
  beforeEach(() => {
    push.mockClear();
  ...
  it('shows no panel until a lead is selected', () => {
    ...
    expect(screen.queryByText(/No theme has been selected/)).not.toBeInTheDocument();
  });
  ...
  it('renders the selected lead’s panel when ?leadId= is already in the URL (reload-safe)', async () => {
    searchParams.set('leadId', 'lead-1');
    global.fetch = vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.includes('/theme') || url.includes('/business')) {
