...
import { BusinessAnalysisDashboardPage } from './business-analysis-dashboard-page';
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <PermissionsProvider role="admin">
        <BusinessAnalysisDashboardPage />
      </PermissionsProvider>
    </QueryClientProvider>,
  );
}

// Select Lead (F6 Dashboard): `leadId` lives in the URL, bookmarkable and
// reload-safe, the same pattern F2's `?period=` and F4's `?q=` already
// established.
describe('BusinessAnalysisDashboardPage', () => {
  beforeEach(() => {
    push.mockClear();
  ...
  it('shows no panel until a lead is selected', () => {
    ...
    expect(
      screen.queryByText('No analysis has been run for this lead yet.'),
