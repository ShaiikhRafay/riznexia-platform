...
import { WebsitePreviewDashboardPage } from './website-preview-dashboard-page';
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <PermissionsProvider role="admin">
        <WebsitePreviewDashboardPage />
      </PermissionsProvider>
    </QueryClientProvider>,
  );
}

describe('WebsitePreviewDashboardPage', () => {
  beforeEach(() => {
    push.mockClear();
  ...
  it('shows no panel until a lead is selected', () => {
    ...
    expect(screen.queryByText(/Preview Status/)).not.toBeInTheDocument();
  });
  ...
  it("renders the selected lead's panel when ?leadId= is already in the URL (reload-safe)", async () => {
    ...
    global.fetch = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes('/website') || url.includes('/preview')) {
