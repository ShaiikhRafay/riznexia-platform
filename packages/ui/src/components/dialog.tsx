...
// F10 (Sales CRM) — first real need for a centered modal (as opposed to
// `Sheet`, which is a side drawer built on the same Radix Dialog
// primitive but visually and behaviorally distinct — full-height,
// slide-in, no natural width cap). Task Create/Edit is this module's own
// first form that belongs in a centered dialog rather than a side panel.
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'border-(--color-border-default) bg-(--color-bg-surface-raised) fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-lg border p-6 shadow-lg',
        className,
      )}
      {...props}
    >
